...
export function parseJsonArrayFromText(text: string) {
  ...
  const jsonBlockMatch = jsonBlockPattern.exec(text);
  ...
  if (jsonBlockMatch) {
    try {
      ...
      if (normalizedJson) jsonData = JSON.parse(normalizedJson);
    } catch (e) {
...
export function parseJSONObjectFromText(
  ...
  if (
    ...
    return parseJsonArrayFromText(text) as Record<string, unknown>;
  } else {
    return null;
  }
}
...
export function extractAttributes(
  ...
  if (!attributesToExtract || attributesToExtract.length === 0) {
    ...
    attributesToExtract.forEach((attribute) => {
      const match = new RegExp(`"${attribute}"\\s*:\\s*"([^"]*)"?`, "i").exec(
        response,
      );
      if (match) {
...
export function cleanMarkdownResponse(response: string): string {
  return response
    .replace(/```markdown\s*/g, "") // Remove ```json
